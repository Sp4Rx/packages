...
public class VideoPlayerPlugin implements FlutterPlugin, AndroidVideoPlayerApi {
    ...
    public @NonNull TextureMessage create(@NonNull CreateMessage arg) {
        TextureRegistry.SurfaceTextureEntry handle = _createHandle();
        EventChannel eventChannel =
                _createEventChannel(handle);

        VideoDataSource videoDataSource = _createVideoDataSource(arg);

        VideoPlayer player = new VideoPlayer(flutterState.applicationContext, eventChannel, handle, videoDataSource, options);
    ...
    @Override
    public @NonNull TextureMessage multiCreate(@NonNull List<CreateMessage> arg) {
        TextureRegistry.SurfaceTextureEntry handle =
                _createHandle();
        EventChannel eventChannel = _createEventChannel(handle);

        List<VideoDataSource> videoDataSources = new ArrayList<>();
        for (CreateMessage createMessage : arg) {
            videoDataSources.add(_createVideoDataSource(createMessage));
        }

        VideoPlayer player = new VideoPlayer(flutterState.applicationContext, eventChannel, handle, videoDataSources, options);
        videoPlayers.put(handle.id(), player);

        return new TextureMessage.Builder().setTextureId(handle.id()).build();
    }

    private TextureRegistry.SurfaceTextureEntry _createHandle() {
        return flutterState.textureRegistry.createSurfaceTexture();
    }

    private EventChannel _createEventChannel(TextureRegistry.SurfaceTextureEntry handle) {
        return new EventChannel(
                flutterState.binaryMessenger, "flutter.io/videoPlayer/videoEvents" + handle.id());
    }

    private VideoDataSource _createVideoDataSource(CreateMessage arg) {
